...
// Upload multipart tronqué par post_max_size : PHP vide $_POST/$_FILES sans erreur exploitable.
// Ne concerne que multipart/form-data — les bodies JSON (lus via php://input) sont hors $_POST/$_FILES par nature.
$content_type = $_SERVER['CONTENT_TYPE'] ?? '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && stripos($content_type, 'multipart/form-data') === 0
...
match(true) {
    ...
    $resource === 'jobs' && $sub === 'gallery'        => require __DIR__ . '/routes/jobs.php',
    $resource === 'jobs' && $sub === 'gift'           => require __DIR__ . '/routes/jobs.php',
    $resource === 'jobs' && $sub === 'messages'       => require __DIR__ . '/routes/jobs.php',

    // ── Galerie publique ──────────────────────────────────────
    $resource === 'gallery'                           => require __DIR__ . '/routes/gallery.php',
